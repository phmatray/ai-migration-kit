...
# `plugins/` is GENERATED output (ADR 0017, #619): every file under it is a byte-for-byte copy of a
# tree entry this check already scans at its source, and `scripts/host-adapters.py check` is what
# holds the copy to that source. Scanning both would report one unmarked spelling twice, and would
# make an AGREED pin's witness count depend on how many plugins happen to copy the file.
GENERATED_ROOTS = ("plugins/",)


def repo_files(repo):
    """Every text file to scan, git-first — minus `GENERATED_ROOTS`.
    ...
    try:
        ...
        names = [n for n in out.decode("utf-8", "replace").split("\0")
                 if n and not n.startswith(GENERATED_ROOTS)]
        if names:
            return names
    ...
    for dirpath, dirnames, filenames in os.walk(repo):
        dirnames[:] = [
            ...
            and not (pathlib.Path(dirpath) == pathlib.Path(repo) and f"{d}/" in GENERATED_ROOTS)
