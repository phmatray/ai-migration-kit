...
6. Write `migration/report.md` following `report-template.md` (sections obligatoires, dont **Prochaines étapes** en checklist actionnable) :
   ...
   - **Santé des dépendances**: the mandatory section of `report-template.md`, rendered from step 4's `dependencyHealth` block — one table row per finding (paquet · version · type · sévérité · avis),
   ...
   - Follow-ups: behavior quirks found (from characterization tests), deferred modernizations, packages held back and why — including any dependency finding the owner decided not to act on now, **with the reason**, so `followups` (which drains this queue) carries it forward rather than losing it.
