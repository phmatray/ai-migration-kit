...
2. Full test run, collecting coverage into `coverage/` the way `templates/ci-dotnet.yml`'s "Tests + couverture" step does: `dotnet test --nologo -- --coverage --coverage-output-format cobertura --results-directory "$PWD/coverage"` — all green; count ≥ the phase-2 baseline count (tests were added, never removed). Without the coverage flags `coverage/` never gets created, and phase 6 dies at the report step (step 5). **Record the counted number and the test platform** (VSTest, or Microsoft Testing Platform when the phase-5 xunit.v3 item ran) — a test count is only meaningful next to the host that produced it, and this is the number that proves the suite still executes rather than merely compiles.
