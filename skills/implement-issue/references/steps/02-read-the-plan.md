...
- **zero, or more than one** → you cannot tell where the task's work belongs, and picking one is the
  ...
  not be re-anchored and what the search returned. If the missing path is created by another, still
  open issue's plan, name that issue's number in the report's `BLOCKED_BY:` field rather than `none`
  — an `auto-dev` worker's supervisor writes it back as a hold instead of a blind re-dispatch.
