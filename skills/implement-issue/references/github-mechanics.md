...
| `guarded-push: ALERT — … HEAD moved while it ran` (exit 4) | HEAD changed between the pre-flight assert and the push. `git push` sends the **current** branch, so what reached the remote may be another task's work. Caught before the remote is read at all — this 4 is about your local checkout, not about the remote | Recover as in the wrong-branch row above: find where the commit went before pushing again, and **never force-push a branch you do not own**. If the message also says the repo "can no longer be read as a git repository", HEAD may never have moved — a worktree removed or renamed under the command reads the same way |
| `guarded-push: ALERT — … could not be listed` / `push is UNVERIFIED` (exit 4) | **Verification never ran.** `git ls-remote` failed — a `--remote` naming a remote the push did not write to, a network drop, an expired credential — so nothing about the remote was read. The push itself exited 0 and *nothing here disproves it* (#93) | **Do not re-push on this code alone** — the work is very likely already on the remote and only the check failed; that is what the ALERT's "treat the work as unpushed" means, *don't assume it landed*, not *it didn't*. Fix the check (pass `--remote <name>` for the remote you actually pushed to, restore connectivity/auth) and **re-run the guard to verify**. The ALERT quotes git's own stderr — read it, it says which |
