...
| `guarded-commit: REFUSED — cannot read the branch assertion` (exit 2) | Both guards source `_assert-branch.sh` from beside themselves, so the invariant has one home (#44). The file is absent or unreadable — a partial install, or a guard copied out of the kit on its own | **Nothing was written.** The guards are not standalone files. Reinstall the plugin, or put `_assert-branch.sh` back in `skills/implement-issue/scripts/`. Do not work around it by calling `git commit` directly — that is the unguarded call the guard replaced |
