...
| `guarded-merge: ALERT — the merge ran, but HEAD is now …` (exit 3) | HEAD moved *during* the merge; `main` was merged into the branch the message names | Do not resolve anything from here. Recover on that branch (`git merge --abort` if it is still in progress, otherwise reset it to its pre-merge tip) — **only if you own it** — then re-run the sync from your own worktree |
