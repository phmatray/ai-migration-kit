...
`skills/merge-pr/scripts/guarded-pr-merge.sh <PR> -- --squash --delete-branch` and decide on
its exit code, never on a bare `gh pr merge`'s — the same decision `merge-pr` Step 5 now runs (#184),
kept in one home so this branch cannot carry its own drifted copy. `0` (MERGED) → the merge landed on
GitHub, whatever `gh pr merge`'s own exit code said (this kit's normal layout — the worker's
`implement-issue` worktree still holding the head branch while the supervisor sits on `main` — makes
its local cleanup half fail routinely); shut the worker down and refill the slot. `1` (QUEUED) → still
`OPEN` but the merge call itself exited 0, a successful merge-queue enqueue, not a rejection; leave the
slot held and re-check next heartbeat, do not retry the merge. `2` (REJECTED) or `3` (CLOSED) → a real
problem the takeover didn't cause; stop and surface it rather than retrying blindly or forcing it
through. `4` (UNCONFIRMED) → the state readback itself didn't answer; do **not** shut the worker down
or refill the slot on this alone — re-run the takeover next heartbeat instead of guessing.
