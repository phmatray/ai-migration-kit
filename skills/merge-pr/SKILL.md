...
3. **An ADR the diff touched.** If the diff touches a path an accepted ADR names in its `code_refs`, run `suggest_adr_from_change` over `git diff main...HEAD` through the `adr` server and add the returned draft to the follow-ups as *ADR proposal*; without the server, grep `docs/adr/*.md` frontmatter for a `code_refs` path this diff touches and write the proposal by hand from the ADR it names, saying AdrMcp was not connected. It is a **proposal for the owner** — it lands under the PR's `## Follow-ups` heading and nowhere else: do not `create_adr` it, do not `set_status` anything, and do not edit the ADR here. The repo profile's *ADRs* section names the root; `none` means skip this source and say so.
