...
#
#   docs/      documentation ABOUT the kit — a consumer's cached skill never reads it
#   tests/     the kit's own golden tests — never executed from an install
...
#   .claude/   this repo's own agent config (repo-profile, worktrees). A consumer's plugin is
#              loaded from .claude-plugin/, never from here.
...
#
# The last two are release-please's own bookkeeping, and excluding them is load-bearing rather than
# cosmetic: its release PR is titled `chore(main): release X.Y.Z` by construction and touches
# exactly .claude-plugin/plugin.json + .release-please-manifest.json + CHANGELOG.md. Gate those and
# `chore` is refused, so no release PR can ever merge — the gate would deadlock the very mechanism
# it exists to protect. Both files are regenerated by the release process; a human editing the
# version by hand is already wrong for other reasons.
#   .release-please-manifest.json  release-please's version ledger
#   .claude-plugin/plugin.json     the version release-please bumps (marketplace.json stays gated)
NON_SHIPPED="docs/ tests/ evals/ reviews/ samples/ .github/ .claude/
