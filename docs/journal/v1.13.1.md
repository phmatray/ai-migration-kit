---
title: "v1.13.1: nine fixes, and a patch number that undersells them"
parent: Journal
nav_order: 13
---

# v1.13.1: nine fixes, and a patch number that undersells them

*Released 2026-08-20.* Cut the same day as v1.13.0, and not a hotfix for it: nine independent
corrections that all happened to be typed `fix`, including the one that stopped fleet workers from
ending their turn to wait for something that would never wake them.
...
types and all nine commits were `fix`, so the story the semantic version tells is "we corrected
v1.13.0", and that is not what happened at all: v1.13.0 shipped `setup-repo`, and not one of these
nine commits touches it. The batch is the backlog of correctness work that happened to clear on the
same day.
